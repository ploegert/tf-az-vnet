...
resource "azurerm_network_watcher" "nwatcher" {
  ...
  name                = "${var.network_watcher_name}_${local.location}"
  location            = local.location
  resource_group_name = local.resource_group_name
...
resource "azurerm_network_security_group" "nsg" {
  for_each            = var.subnets
  name                = lower("${var.nsg_name}_${each.key}")
  resource_group_name = local.resource_group_name
  location            = local.location
...
resource "azurerm_private_dns_zone" "openblue" {
...
resource "azurerm_private_dns_zone_virtual_network_link" "ob_dns_link" {
    #count                 = var.create_private_dns ? 1 : 0
    count                   = var.dns != null ? 1 : 0
    name                  = var.dns.link_name
    resource_group_name   = local.resource_group_name
    private_dns_zone_name = azurerm_private_dns_zone.openblue[count.index].name
    virtual_network_id    = azurerm_virtual_network.vnet.id
    registration_enabled  = var.dns.vnet_auto_registration
}
